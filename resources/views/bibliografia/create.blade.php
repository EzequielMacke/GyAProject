...
<head>
    ...
    <title>Nueva Bibliografía</title>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .btn-elemento {
            height: 34px; padding: 0 0.85rem; border-radius: 0.5rem;
        ...
        .detalle-item-label {
            ...
            white-space: nowrap; padding-top: 0.15rem; min-width: 80px;
        }
        .detalle-item-input { flex: 1; }
        .detalle-item-input .form-control { background: var(--surface); }
        .detalle-item-remove {
            background: none; border: none; cursor: pointer;
            color: var(--muted); font-size: 0.8rem; padding: 0.2rem;
            border-radius: 0.3rem; transition: color 0.14s; flex-shrink: 0;
            margin-top: 0.1rem;
        }
        .detalle-item-remove:hover { color: #c0392b; }
        .drag-handle {
            color: var(--border2); cursor: grab; font-size: 0.78rem;
            padding: 0.1rem 0.3rem 0.1rem 0; flex-shrink: 0; margin-top: 0.15rem;
        ...
        .file-upload-area {
            border: 2px dashed var(--border2); border-radius: 0.65rem;
        ...
        .empty-elementos {
            ...
            border: 1.5px dashed var(--border); border-radius: 0.65rem;
        }
        ...
        .alert-error {
            background: #fff0f0; color: #c0392b; border: 1px solid #f5c2c2;
        ...
        .nota-pie-aviso {
            ...
            background: #fffbea; border: 1px solid #e8cc50; border-radius: 0.45rem;
            padding: 0.5rem 0.7rem; font-size: 0.74rem; color: #7a5800;
    ...
    <div class="content-wrapper">
            ...
            <div class="container-fluid">
                <div class="ph">
                    <div>
                        ...
                        <h1 class="ph-title">Nueva <em>Bibliografía</em></h1>
                        <p class="ph-sub">Formulario para creación de una nueva bibliografía</p>
                    </div>
                    <div class="ph-right">
        ...
        <section class="content">
            <div class="container-fluid">
                ...
                <form method="POST" action="{{ route('bibliografia.store') }}" enctype="multipart/form-data">
                    @csrf
                        ...
                        <div class="section-body">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="nombre"
                                    value="{{ old('nombre') }}" placeholder="Nombre de la bibliografía" required>
                            </div>
                            <div class="form-group">
                                <label class="form-label">Fuente</label>
                                <input type="text" class="form-control" name="fuente"
                                    value="{{ old('fuente') }}" placeholder="Fuente" required>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="section-card">
                        ...
                        <div class="section-body">
                            ...
                            {{-- Botones de elementos disponibles --}}
                            <div class="elementos-btns" style="margin-top:1.25rem;margin-bottom:0;">
                                ...
                                <button type="button" class="btn-elemento"
                                    ...
                                    data-estilo="{{ $el->estilo }}"
                                    onclick="agregarElemento(this)">
                                    <i class="fas fa-plus"></i> {{ $el->nombre }}
                                </button>
                                @endforeach

                                @if($elementos->where('id', '!=', 9)->isEmpty())
                                <span style="font-size:0.82rem;color:var(--muted);">
                                    No hay elementos disponibles. Agregá elementos en el sistema primero.
                                </span>
                                @endif
                            </div>
...
<script>
    ...
    function toggleNotaPie(checkbox, idx) {
        const panel    = document.getElementById('np-panel-' + idx);
        const textarea = document.getElementById('np-textarea-' + idx);
    ...
    function eliminarItem(btn) {
        ...
        if (lista.children.length === 0) {
            ...
            empty.id = 'empty-msg';
            empty.innerHTML = '<i class="fas fa-layer-group" style="font-size:1.5rem;margin-bottom:0.5rem;display:block;"></i>Usá los botones de arriba para agregar contenido';
