...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .btn-icon-danger:disabled { opacity: 0.3; cursor: not-allowed; pointer-events: none; }
    ...
    <div class="content-wrapper">
                ...
                <div class="ph">
                    <div>
                        <div class="ph-crumb">
                            ...
                            @if($obraModel)
                                <a href="{{ route('obras.show', $pedido->obra_id) }}">{{ $obraModel->nombre }}</a>
                                <i class="fas fa-chevron-right"></i>
                        ...
                        <h1 class="ph-title">Editar <em>pedido #{{ $pedido->id }}</em></h1>
                        <p class="ph-sub">{{ $obraModel->nombre ?? 'Pedido de insumos para obra' }}</p>
                    </div>
                    <div class="ph-right">
        ...
        <section class="content">
            <div class="container-fluid">
                ...
                <form id="form-edit" action="{{ route('pedidobra.update', $pedido->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="obra" value="{{ $pedido->obra_id }}">
                    <input type="hidden" name="contador_insumos" id="contador_insumos_input" value="{{ $pedido->total_insumo }}">
                        ...
                        <div class="form-card-body">
                            <div class="fields-grid">
                                <div>
                                    ...
                                    <input type="text" class="field-input" value="{{ $pedido->id }}" readonly>
                                </div>
                                <div>
                                    <label class="field-label">Creado por</label>
                                    <input type="text" class="field-input" value="{{ $pedido->usuario->nombre ?? session('usuario_nombre') }}" readonly>
                                </div>
                                <div>
                                    <label class="field-label">Fecha de pedido</label>
                                    <input type="date" name="fecha_pedido" class="field-input" value="{{ $pedido->fecha_pedido }}" readonly>
                                </div>
                                <div>
                                    <label class="field-label" for="fecha_entrega">Fecha de entrega</label>
                                    <input type="date" name="fecha_entrega" id="fecha_entrega" class="field-input"
                        ...
                        <div class="form-card-body">
                            <div class="fields-grid">
                                <div>
                                    ...
                                    @if($obraModel)
                                        <div class="obra-chip">
                                            <i class="fas fa-building"></i> {{ $obraModel->nombre }}
                                        </div>
                                    @else
                                        <input type="text" class="field-input" value="Sin obra asignada" readonly>
                                    @endif
                                </div>
                                <div>
                                    ...
                                    <select name="presupuesto_aprobado_id" id="presupuesto_aprobado_id" class="field-input" required>
                                        ...
                                        @foreach($presupuestos as $p)
                                            <option value="{{ $p->id }}"
                                                    data-orden="{{ $p->orden_trabajo ?? '' }}"
                                                    {{ old('presupuesto_aprobado_id', $pedido->presupuesto_aprobado_id) == $p->id ? 'selected' : '' }}>
                                                {{ $p->clave }}
                                            </option>
                                        @endforeach
                                    </select>
                    ...
                    <div class="form-card">
                        ...
                        <div class="form-card-body">
                            ...
                            <textarea name="observacion" id="observacion" class="field-input"
                                      placeholder="Notas adicionales…">{{ old('observacion', $pedido->observacion) }}</textarea>
                        </div>
                    </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    presupuestoSel.addEventListener('change', syncOrden);
    syncOrden();
    ...
    function buildRow(id, name, cantidad, unidadMedida, confirmed) {
        const n  = tbody.querySelectorAll('tr.insumo-row').length + 1;
        const tr = document.createElement('tr');
        tr.className = 'insumo-row';
        tr.draggable = true;
        // Para filas confirmadas: select deshabilitado con hidden duplicado para que se envíe
        const unitCell = confirmed
            ? `<select class="field-input" disabled>${buildUnidades(unidadMedida)}</select>
               <input type="hidden" name="unidad_medida[]" value="${unidadMedida}">`
            : `<select name="unidad_medida[]" class="field-input" draggable="false">${buildUnidades(unidadMedida)}</select>`;
        const removeBtn = confirmed
            ? `<button type="button" class="btn-icon-danger remove-btn" disabled><i class="fas fa-times"></i></button>
               <span class="confirmed-badge" style="margin-left:0.3rem;"><i class="fas fa-check"></i> Listo</span>`
            : `<button type="button" class="btn-icon-danger remove-btn" draggable="false"><i class="fas fa-times" draggable="false"></i></button>`;
        tr.innerHTML = `
            ...
            <td>${unitCell}</td>
            <td><input type="number" name="cantidad[]" class="field-input" value="${cantidad}" min="0.01" step="0.01" draggable="false"${confirmed ? ' readonly' : ''}></td>
    ...
    (function() {
        const confirmed = {{ $detalle->confirmado == 2 ? 'true' : 'false' }};
        tbody.appendChild(buildRow(
            {{ $detalle->insumo_id }},
            @js($detalle->insumo->nombre ?? ''),
            {{ $detalle->cantidad }},
            @js((string) $detalle->medida),
            confirmed
        ));
    })();
    @endforeach
    ...
    document.getElementById('add-insumo-btn').addEventListener('click', function () {
        ...
        if (existing) {
            ...
            if (!cantInput.readOnly) cantInput.value = (parseFloat(cantInput.value) || 0) + 1;
            clearInsumoSel(); return;
        }
        tbody.appendChild(buildRow(id, name, 1, '', false));
        clearInsumoSel();
        refreshCounter();
    });
    ...
    document.getElementById('add-kit-btn').addEventListener('click', function () {
        ...
        kit.detalles.forEach(det => {
            ...
            if (existing) {
                const cantInput = existing.closest('tr').querySelector('input[name="cantidad[]"]');
                if (!cantInput.readOnly) cantInput.value = (parseFloat(cantInput.value) || 0) + cantidad;
            } else {
                tbody.appendChild(buildRow(id, name, cantidad, unidad, false));
            }
        });
        clearKitSel();
        refreshCounter();
