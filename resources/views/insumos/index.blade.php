...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .ins-table tbody tr:hover { background: var(--surface2); }
        ...
        .user-cell { display: flex; align-items: center; gap: 0.38rem; font-size: 0.8rem; color: var(--muted); }
        .user-cell i { font-size: 0.62rem; }
    ...
    <div class="content-wrapper">
                ...
                <div class="ph">
                    <div>
                        <div class="ph-crumb">
                            ...
                            <i class="fas fa-chevron-right"></i> Insumos
                        </div>
                        <h1 class="ph-title">Listado de <em>Insumos</em></h1>
                        <p class="ph-sub">Visualizá y gestioná todos los insumos disponibles</p>
                    </div>
                    <div class="ph-right">
                        <div class="search-wrap">
                            <i class="fas fa-search"></i>
                            <input type="text" id="search" class="search-bar" placeholder="Buscar insumos…" autocomplete="off">
                        </div>
                        @permiso('ins', 'agregar')
                        <a href="{{ route('insumos.create') }}" class="btn btn-primary" id="agregar-insumo-btn">
        ...
        <section class="content">
            <div class="container-fluid">
                ...
                @php
                    $total     = $insumos->count();
                    $activos   = $insumos->where('estado', 1)->count();
                ...
                <div class="table-wrap">
                    <table class="ins-table" id="ins-table">
                        <thead>
                            <tr>
                                <th class="row-num">#</th>
                                <th>Nombre</th>
                        ...
                        <tbody>
                            @forelse($insumos->reverse()->values() as $insumo)
                            @php
                                $on       = $insumo->estado == 1;
                                $initials = mb_strtoupper(mb_substr($insumo->nombre, 0, 2));
                            @endphp
                            <tr data-search="{{ strtolower($insumo->nombre . ' ' . ($insumo->usuario->nombre ?? '')) }}"
                                ...
                                <td>
                                    <div class="name-cell">
                                        ...
                                        <span class="name-text">{{ $insumo->nombre }}</span>
                                    </div>
                                </td>
                                <td>
                                    <div class="user-cell">
                                        <i class="fas fa-user"></i>
                                        {{ $insumo->usuario->nombre ?? '—' }}
                                    </div>
                                </td>
                                <td>
                                    <span class="status-badge {{ $on ? 'on' : 'off' }}">
                                        <i class="fas fa-circle"></i>
                                        {{ $estados[$insumo->estado] ?? 'Desconocido' }}
                                    </span>
                                </td>
                            </tr>
                            @empty
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const rows  = document.querySelectorAll('#ins-table tbody tr[data-search]');
    ...
    document.addEventListener('keydown', function (e) {
        if (e.ctrlKey && e.key === '1') {
            e.preventDefault();
            const btn = document.getElementById('agregar-insumo-btn');
            if (btn) btn.click();
        }
    });
});
