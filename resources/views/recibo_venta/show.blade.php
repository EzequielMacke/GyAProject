...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .card-created i { font-size: 0.6rem; }
            ...
            <div class="container-fluid">
                ...
                <div class="ph">
                    <div>
                        ...
                        <h1 class="ph-title">Recibos — <em>{{ $factura?->nro_factura ?? 'Factura' }}</em></h1>
                        <p class="ph-sub">Cobros registrados para esta factura</p>
                    </div>
                    <div class="ph-right">
                        <div class="search-wrap">
                            <i class="fas fa-search"></i>
                            <input type="text" id="search" class="search-bar" placeholder="Buscar recibo…" autocomplete="off">
                        </div>
                        <a href="{{ route('factura_venta.index', ['presupuesto' => $presupuesto, 'obra' => $obra]) }}" class="btn">
        ...
        <section class="content">
            <div class="container-fluid">
                ...
                @php
                    ...
                    $porcentajeCobrado = $montoFactura > 0 ? round(($cobradoTotal / $montoFactura) * 100, 1) : 0;
                    $saldoPorCobrar    = $montoFactura - $cobradoTotal;
                @endphp

                <div class="summary-strip">
                    <div class="summary-item">
                        <span class="summary-label">Nro. Factura</span>
                        <span class="summary-val muted">{{ $factura?->nro_factura ?? '—' }}</span>
                    </div>
                    <div class="summary-item">
                        <span class="summary-label">Monto factura</span>
                        <span class="summary-val blue">Gs. {{ number_format($montoFactura, 0, '', '.') }}</span>
                    </div>
                    <div class="summary-item">
                        <span class="summary-label">Cobrado</span>
                        <span class="summary-val teal">Gs. {{ number_format($cobradoTotal, 0, '', '.') }} ({{ $porcentajeCobrado }}%)</span>
                    </div>
                    <div class="summary-item">
                ...
                <div id="cards-grid">
                    ...
                    @php
                        ...
                        $porcentajeRecibo   = $montoFacturaRecibo > 0
                            ? round(($recibo->monto / $montoFacturaRecibo) * 100, 1)
                            : 0;
                        $searchData = strtolower(
                            ($recibo->nro_recibo ?? '') . ' ' .
                            ($recibo->concepto ?? '') . ' ' .
                            number_format($recibo->monto, 0, '', '.') . ' ' .
                    ...
                    <div class="recibo-card"
                        ...
                        <div class="card-head">
                            <div class="card-head-top">
                                <div>
                                    <div class="card-nro-label">Nro. Recibo</div>
                                    <div class="card-nro">{{ $recibo->nro_recibo }}</div>
                                </div>
                            ...
                            @if($recibo->concepto)
                            <div class="card-concepto">{{ $recibo->concepto }}</div>
                            @endif
                        </div>
                        ...
                        <div class="card-body">
                            ...
                            <div class="card-monto-row">
                                ...
                                <span class="card-monto">Gs. {{ number_format($recibo->monto, 0, '', '.') }}</span>
                            </div>

                            <div class="pct-block">
                                <span class="pct-block-label">% de la factura</span>
                                <span class="pct-block-val">
                                    {{ $porcentajeRecibo }}%
                                    <span style="font-size:0.7rem; color:var(--muted); font-family:'Plus Jakarta Sans',sans-serif;">
                                        de Gs. {{ number_format($montoFacturaRecibo, 0, '', '.') }}
                                    </span>
                                </span>
                            </div>
                            ...
                            <div class="prog-row">
                                <div class="prog-label-row">
                                    ...
                                    <span class="prog-val">{{ $porcentajeRecibo }}%</span>
                                </div>
                                <div class="prog-track">
                                    <div class="prog-fill" style="width:{{ min($porcentajeRecibo, 100) }}%"></div>
                                </div>
                            </div>
                            ...
                            <div class="card-created">
                                ...
                                Creado {{ $recibo->created_at->diffForHumans() }}
                            </div>
                            @endif
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const cards = document.querySelectorAll('#cards-grid .recibo-card');
