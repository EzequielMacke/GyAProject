...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .pdf-filename a { color: var(--accent); text-decoration: none; font-weight: 600; }
        .pdf-filename a:hover { text-decoration: underline; }
    </style>
</head>
    ...
    <div class="content-wrapper">
            ...
            <div class="container-fluid">
                <div class="ph">
                    <div>
                        ...
                        <h1 class="ph-title">Editar <em>presupuesto</em></h1>
        ...
        <section class="content">
            <div class="container-fluid">
                ...
                <form id="form-edit" action="{{ route('presupuesto_aprobado.update', $presupuesto->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="obra_id" value="{{ $presupuesto->obra_id ?? '' }}">
                            ...
                            <div class="fields-grid">
                                <div>
                                    ...
                                    <input type="text" id="clave" name="clave" class="field-input"
                                           ...
                                           value="{{ old('clave', $presupuesto->clave) }}">
                                </div>
                                <div>
                                ...
                                <div>
                                    <label class="field-label" for="orden_trabajo">Orden de trabajo</label>
                                    <input type="text" id="orden_trabajo" class="field-input"
                                           value="{{ $presupuesto->orden_trabajo ?? '-' }}" disabled>
                                </div>
                                <div>
                                    ...
                                    <input type="text" id="monto_total" name="monto_total" class="field-input"
                                           ...
                                           value="{{ old('monto_total', number_format($presupuesto->monto_total, 0, '', '.')) }}">
                                </div>
                                <div>
                                    ...
                                    <select id="tipo_trabajo" name="tipo_trabajo" class="field-input" required>
                                        ...
                                        @foreach (config('constantes.tipo_trabajo') as $codigo => $tipo)
                                            <option value="{{ $codigo }}" {{ old('tipo_trabajo', $presupuesto->tipo_trabajo) == $codigo ? 'selected' : '' }}>
                                                {{ $tipo }}
                                            </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="form-card-body">
                            <div class="pdf-grid">
                                <div class="pdf-drop-wrap">
                                    ...
                                    <div class="pdf-square{{ $presupuesto->presupuesto ? ' has-file' : '' }}"
                                         id="sq-presupuesto" tabindex="0" role="button">
                                        <input type="file" name="presupuesto" id="presupuesto" accept="application/pdf" class="d-none">
                                        <div class="pdf-square-icon"><i class="fas fa-file-pdf"></i></div>
                                        <div class="pdf-square-label">{{ $presupuesto->presupuesto ? 'Reemplazar archivo' : 'Seleccionar PDF' }}</div>
                                        <div class="pdf-square-sub">o arrastrá aquí</div>
                                    </div>
                                    <div class="pdf-filename" id="fn-presupuesto">
                                        @if($presupuesto->presupuesto)
                                            <a href="{{ Storage::url('presupuestos/'.$presupuesto->presupuesto) }}" target="_blank">
                                                <i class="fas fa-external-link-alt" style="font-size:0.65rem;"></i>
                                                {{ basename($presupuesto->presupuesto) }}
                                            </a>
                                        @endif
                                    </div>
                                </div>
                                ...
                                <div class="pdf-drop-wrap">
                                    ...
                                    <div class="pdf-square{{ $presupuesto->conformidad ? ' has-file' : '' }}"
                                         id="sq-conformidad" tabindex="0" role="button">
                                        <input type="file" name="conformidad" id="conformidad" accept="application/pdf" class="d-none">
                                        <div class="pdf-square-icon"><i class="fas fa-file-pdf"></i></div>
                                        <div class="pdf-square-label">{{ $presupuesto->conformidad ? 'Reemplazar archivo' : 'Seleccionar PDF' }}</div>
                                        <div class="pdf-square-sub">o arrastrá aquí</div>
                                    </div>
                                    <div class="pdf-filename" id="fn-conformidad">
                                        @if($presupuesto->conformidad)
                                            <a href="{{ Storage::url('conformidades/'.$presupuesto->conformidad) }}" target="_blank">
                                                <i class="fas fa-external-link-alt" style="font-size:0.65rem;"></i>
                                                {{ basename($presupuesto->conformidad) }}
                                            </a>
                                        @endif
                                    </div>
                                </div>
                    ...
                    <div class="form-card">
                        ...
                        <div class="form-card-body">
                            ...
                            <textarea id="observacion" name="observacion" class="field-input"
                                      placeholder="Notas adicionales…">{{ old('observacion', $presupuesto->observacion) }}</textarea>
                        </div>
                    </div>
