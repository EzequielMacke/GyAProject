        ...
        <div class="content-wrapper" style="min-height: 100vh; background: transparent;">
            <div class="container-fluid">
                <div class="card card-custom p-4 w-100" style="min-width:0;">
                    <form action="{{ route('presupuesto_aprobado.update', $presupuesto->id) }}" method="POST" enctype="multipart/form-data" class="w-100">
                        @method('PUT')
                        <div class="row mb-3">
                            <div class="col-8">
                        ...
                        <div class="row mb-3">
                            <div class="col-md-4 mb-2">
                                ...
                                <input type="text" name="clave" class="form-control" id="clave" value="{{ old('clave', $presupuesto->clave) }}" required>
                            </div>
                            <div class="col-md-4 mb-2">
                                <label for="ubicacion" class="form-label">Ubicación del presupuesto</label>
                                <input type="text" name="ubicacion" class="form-control" id="ubicacion" value="{{ old('ubicacion', $presupuesto->ubicacion) }}" required>
                            </div>
                            <div class="col-md-4 mb-2">
                                <label for="orden_trabajo" class="form-label">Orden de trabajo</label>
                                <input type="text" class="form-control" id="orden_trabajo" value="{{ $presupuesto->orden_trabajo ?? '-' }}" disabled>
                            </div>
                        </div>
                        <div class="row mb-3">
                            <div class="col-md-6 mb-2">
                                ...
                                <select name="tipo_trabajo" class="form-select" id="tipo_trabajo" required>
                                    ...
                                    @foreach (config('constantes.tipo_trabajo') as $codigo => $tipo_trabajo)
                                        <option value="{{ $codigo }}" {{ old('tipo_trabajo', $presupuesto->tipo_trabajo) == $codigo ? 'selected' : '' }}>{{ $tipo_trabajo }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="row mb-3">
                            <div class="col-md-6 mb-2">
                                ...
                                <input type="text" name="monto_total" class="form-control" id="monto_total" value="{{ old('monto_total', number_format($presupuesto->monto_total, 0, '', '.')) }}" required>
                            </div>
                        </div>
                                ...
                                <div class="pdf-square-grid">
                                    <div class="d-flex flex-column align-items-center">
                                        ...
                                        <div class="pdf-square" id="sq-presupuesto" tabindex="0" role="button" aria-label="Seleccionar presupuesto PDF">
                                            ...
                                            <div class="label" style="{{ $presupuesto->presupuesto ? 'display:none;' : '' }}">Suelta aquí<br>o selecciona tu archivo</div>
                                            <div class="filename" id="fn-presupuesto">@if($presupuesto->presupuesto)<a href="{{ asset('storage/'.$presupuesto->presupuesto) }}" target="_blank">{{ basename($presupuesto->presupuesto) }}</a>@endif</div>
                                        </div>
                                    </div>
                                    <div class="d-flex flex-column align-items-center">
                                        ...
                                        <div class="pdf-square" id="sq-conformidad" tabindex="0" role="button" aria-label="Seleccionar nota de conformidad PDF">
                                            ...
                                            <div class="filename" id="fn-conformidad">@if($presupuesto->conformidad)<a href="{{ asset('storage/'.$presupuesto->conformidad) }}" target="_blank">{{ basename($presupuesto->conformidad) }}</a>@endif</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-12">
                                ...
                                <textarea name="observacion" class="form-control" id="observacion" rows="4">{{ old('observacion', $presupuesto->observacion) }}</textarea>
                            </div>
                        </div>
                    </form>
                </div>
